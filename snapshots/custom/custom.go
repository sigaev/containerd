...
func (o *snapshotter) Usage(ctx context.Context, key string) (usage snapshots.Usage, err error) {
	...
	if info.Kind == snapshots.KindActive {
		dir := o.getSnapshotDir(id)
		if _, statErr := os.Stat(dir); statErr == nil {
			du, err := fs.DiskUsage(ctx, dir)
			if err != nil {
				return snapshots.Usage{}, err
			}
			usage = snapshots.Usage(du)
		}
		// Child snapshots (with parent) have no own dir; usage stays zero.
	}
...
func (o *snapshotter) createSnapshot(ctx context.Context, kind snapshots.Kind, key, parent string, opts []snapshots.Opt) (_ []mount.Mount, err error) {
	...
	// When a parent is provided the caller (e.g. container creation) just
	// needs a writable view of the already-committed snapshot.  We record
	// the relationship in the metadata store but skip creating a new tmpfs;
	// mounts() will resolve to the parent's directory.
	if parent != "" {
		err = o.ms.WithTransaction(ctx, true, func(ctx context.Context) error {
	...
	// No parent — fresh tmpfs-backed snapshot (the image pull path).
	var path, td string
