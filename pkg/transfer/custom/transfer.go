...
// Register stores the singleton instance. Called by the transfer plugin at init.
func Register(ts *TransferService) {
	instanceMu.Lock()
	defer instanceMu.Unlock()
	instance = ts
}

// GetInstance returns the registered singleton, or nil.
func GetInstance() *TransferService {
	instanceMu.RLock()
...
// TransferService intercepts pull operations and hands them to a
...
func NewTransferService(lm leases.Manager, is images.Store, sn snapshots.Snapshotter, snapshotterName string, fn FetchFunc) *TransferService {
...
// PullImage is the direct entry point for CRI. It performs the black-box
// fetch and commits the result into the snapshotter + image store.
func (ts *TransferService) PullImage(ctx context.Context, imageRef string) (images.Image, error) {
	...
	chainID, err := ts.commitTree(ctx, imageRef)
	if err != nil {
		return images.Image{}, err
	}
	...
	syntheticDesc := ocispec.Descriptor{
		...
		Digest:    digest.FromString(imageRef + chainID),
		Size:      0,
	...
	img := images.Image{
		Name:      imageRef,
		Target:    syntheticDesc,
		CreatedAt: now,
		UpdatedAt: now,
	}

	if created, err := ts.images.Create(ctx, img); err != nil {
		if !errdefs.IsAlreadyExists(err) {
			return images.Image{}, fmt.Errorf("custom pull: failed to create image: %w", err)
		}
		img, err = ts.images.Update(ctx, img)
		if err != nil {
			return images.Image{}, fmt.Errorf("custom pull: failed to update image: %w", err)
		}
	} else {
		img = created
	}
...
func (ts *TransferService) Transfer(ctx context.Context, src interface{}, dst interface{}, opts ...transfer.Opt) error {
...
func (ts *TransferService) pull(ctx context.Context, src transfer.ImageFetcher, dst transfer.ImageStorer, topts *transfer.Config) error {
...
func (ts *TransferService) commitTree(ctx context.Context, imageRef string) (string, error) {
...
func (ts *TransferService) withLease(ctx context.Context, opts ...leases.Opt) (context.Context, func(context.Context) error, error) {
